...
# 读取数据
min_distance_rec = []
with open(r'./result/all_algebraic_conn_rec.txt', 'r') as file:
    for line in file:
        min_distance_rec.append(float(line.strip()))  # 将字符串转换为浮点数
min_distance_rec=min_distance_rec[:3000]
compare_min_distance_rec = []
with open(r'./result/compare_all_algebraic_conn_rec.txt', 'r') as file:
    for line in file:
...
with open(r'./result/xiaorong_all_algebraic_conn_rec.txt', 'r') as file:
    for line in file:
...
axs[0].set_xticks([])
axs[0].set_ylabel('$\u03BB_{2}$')
# 添加网格线
axs[0].grid(True, linestyle='--', alpha=0.7)
axs[0].legend(loc='lower right',fontsize=14,handlelength=1.2)


# 读取数据
min_distance_rec = []
with open(r'./result/all_algebraic_conn_cir.txt', 'r') as file:
    for line in file:
        min_distance_rec.append(float(line.strip()))  # 将字符串转换为浮点数
min_distance_rec=min_distance_rec[:3000]
compare_min_distance_rec = []
with open(r'./result/compare_all_algebraic_conn_cir.txt', 'r') as file:
    for line in file:
        compare_min_distance_rec.append(float(line.strip()))  # 将字符串转换为浮点数
compare_min_distance_rec=compare_min_distance_rec[:3000]
xiaorong_min_distance_rec = []
with open(r'./result/xiaorong_all_algebraic_conn_cir.txt', 'r') as file:
    for line in file:
        xiaorong_min_distance_rec.append(float(line.strip()))  # 将字符串转换为浮点数
xiaorong_min_distance_rec=xiaorong_min_distance_rec[:3000]
...
axs[1].set_ylabel('$\u03BB_{2}$')
axs[1].grid(True, linestyle='--', alpha=0.7)
axs[1].legend(loc='lower right',fontsize=14,handlelength=1.2)

# 保存图形
plt.subplots_adjust(hspace=0.05)
plt.savefig('fig_change_algebraic_connectivity.jpg', dpi=600)  # 设置保存的分辨率
plt.show()
