...
def draw_map(obstacles, params,ax):
    ...
    world_bounds_y = [params.world_bounds_y[0], params.world_bounds_y[1] + 20]
...
# 读取障碍物信息
obstacles = []
with open(r'./result/obstacles_cir.txt', 'r') as file:
    for line in file:
...
import json
import numpy as np
with open(r'./result/trajectory_cir.json', 'r') as file:
    trajectory = json.load(file)
with open(r'./result/compare_trajectory_cir.json', 'r') as file:
    compare_trajectory = json.load(file)
with open(r'./result/xiaorong_trajectory_cir.json', 'r') as file:
...
axs[0].plot(obstacles[-1][0], obstacles[-1][1], 'o', color='#777777', markersize=4, label='Obstacles')
axs[0].legend(
...
axs[1].plot(obstacles[-1][0], obstacles[-1][1], 'o', color='#777777', markersize=4, label='Obstacles')
axs[1].legend(
    loc='upper right',      # 设置图例位置为右上角
    bbox_to_anchor=(1.01, 1.01),  # 锚点位置设置为右上角
    ncol=1,                  # 图例分为 6 列
...
axs[2].plot(obstacles[-1][0], obstacles[-1][1], 'o', color='#777777', markersize=4, label='Obstacles')
...
plt.savefig('fig_trajectory_cir.jpg', dpi=600)  # 设置保存的分辨率
